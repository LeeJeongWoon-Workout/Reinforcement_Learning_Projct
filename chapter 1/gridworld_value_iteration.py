...
def print_values(V, grid): # V is dictionary contatining values for each node
...
def print_policy(P, grid):  #P is dictionary containing policy (action chice) for each node
	for i in range(grid.rows):
		print("---------------------------")
		for j in range(grid.cols):
			action = P.get((i, j), ' ')
